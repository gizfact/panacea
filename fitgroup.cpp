...
void __fastcall TFitGroupForm::ToolButton2Click(TObject *Sender)
{
    ...
    if(atoi(sRet.c_str()) != 0)
    {
        if(Application->MessageBox("Внимание!\nНа услугу есть записи. Удалить услугу и связанные записи?","",MB_OKCANCEL) == IDCANCEL)
            return;
...
{
    // Копирование предыдущего расписания
