...
static int fitshedule_fill(void *NotUsed,int argc,char **argv,char **azColName)
{
    ...
    if(ATOF(argv[3]) + (15.0 / 1440.0) < (double)Now())
    {
    ...
    else
    {
        ...
        ClientsForm->sgFitShedule->Cells[0][pos] = ATOI(argv[6])? " " : "";
    }
    ...
    SQL_exefun(DBName,("select count(*) from Reserve where FitGroupID="+AnsiString(ID)).c_str(),&sRet);
    ClientsForm->sgFitShedule->Cells[4][pos] = ATOI(sRet.c_str());
...
static int abonements_bath_select(void *NotUsed, int argc, char **argv, char **azColName)
{
    ...
    if(ClientsForm->sgAbonementsBath->Cells[0][CounterAbonementsBath].Length() == 0 && (int)dt < (int)Now())
        ClientsForm->sgAbonementsBath->Cells[0][CounterAbonementsBath] = "\t";
    // Price - argv[5]

    SQL_exefun(DBName,((AnsiString)"select count(*) from Visits_Bath where AbonementID=" + argv[0]).c_str(),&retString);
    double price, amount;
    if(argv[5])
        price = atof(argv[5]);
    else
    ...
    if(fixsum >= 0.0 && !visits)
    {
        ClientsForm->sgAbonementsBath->Cells[4][CounterAbonementsBath] = FormatFloat(MoneyFormat,price).TrimLeft();
        ClientsForm->sgAbonementsBath->Cells[5][CounterAbonementsBath] = atoi(retString.c_str());
    ...
    {
        ClientsForm->sgAbonementsBath->Cells[4][CounterAbonementsBath] = FormatFloat(MoneyFormat,price).TrimLeft();
        ClientsForm->sgAbonementsBath->Cells[5][CounterAbonementsBath] = "";
    ...
    ClientsForm->sgAbonementsBath->Cells[4][CounterAbonementsBath] = FormatFloat(MoneyFormat,amount).TrimLeft() + " / " + FormatFloat(MoneyFormat,price).TrimLeft();
    ClientsForm->sgAbonementsBath->Cells[5][CounterAbonementsBath] = (AnsiString)z + " / " + visits;
...
{
    if(ClientsForm->IDs)
        ClientsForm->IDs[ClientsForm->Counter] = _atoi64(argv[0]);
    else
        return 0;

    if(argv[1])
        ClientsForm->sgClients->Cells[0][++ClientsForm->Counter] = (atoi(argv[1]))? "" : " ";
    else
        ClientsForm->sgClients->Cells[0][++ClientsForm->Counter] = "";

    ClientsForm->sgClients->Cells[1][ClientsForm->Counter] = argv[2];
    ClientsForm->sgClients->Cells[2][ClientsForm->Counter] = argv[3];
    ClientsForm->sgClients->Cells[3][ClientsForm->Counter] = argv[4];
    ClientsForm->sgClients->Cells[4][ClientsForm->Counter] = argv[5];
...
//---------------------------------------------------------------------------
void __fastcall TClientsForm::sgAbonementsBathFullUpdate(__int64 ID,__int64 LastAbID)
{
    if(SQL_BeginTransaction() != SQLITE_OK) return;
    ...
    // Отдельно не абонементы
    unsigned cnt = SQL_fldKeyCollect(DBName,"EndDate",("from Abonements_Bath where Closed=0 and AbonementsCount<1 and ClientID="+AnsiString(ClientID)).c_str(),&AbIDs,&pAbCounts);
    ...
    cnt = SQL_fldKeyCollect(DBName,"EndDate",("from Abonements_Bath where Closed=0 and AbonementsCount=1 and ClientID="+AnsiString(ClientID)).c_str(),&AbIDs,&pAbCounts);
    ...
    for(unsigned i = 0; i < cnt; i++)
    {
        ...
        {
            SQL_exefun(DBName,("select count(*) from Visits_Bath where AbonementID="+AnsiString(AbIDs[i])).c_str(),&retString);
            if(atoi(retString.c_str()) >= 1)
                SQL_exe(DBName,("update Abonements_Bath set Closed=1 where RowID="+AnsiString(AbIDs[i])).c_str());
        }
    }
    ...
    cnt = SQL_fldKeyCollect(DBName,"AbonementsCount",("from Abonements_Bath where Closed=0 and AbonementsCount>1 and ClientID="+AnsiString(ClientID)+" order by BegDate").c_str(),&AbIDs,&pAbCounts);
    ...
    for(unsigned i = 0; i < cnt; i++)
    {
        ...
        // Есть след. абонемент!
        SQL_exefun(DBName,("select EndDate from Abonements_Bath where RowID="+AnsiString(AbIDs[i])).c_str(),&sEndDate);
        SQL_exefun(DBName,("select CDate from Abonements_Bath where RowID="+AnsiString(AbIDs[i+1])).c_str(),&retString);
        if((int)addmonths(atoi(sEndDate.c_str()),2) < atoi(retString.c_str()))
        {
            // закрыть
            SQL_exe(DBName,("update Abonements_Bath set Closed=1 where RowID="+AnsiString(AbIDs[i])).c_str());
    ...
    {
        if(LastAbID)
            SQL_exefun(DBName,("select count(*) from Abonements_Bath where (Closed=0 or RowID="+AnsiString(LastAbID)+") and ClientID="+(AnsiString)ID).c_str(),&retString);
        else
    ...
    if(icnt)
    {
        ...
        //SQL_exe(DBName,("select Bills.RowID,CDate,sum(Total) from Bills inner join BillItems on Bills.RowID=BillID where ClientID="+(AnsiString)ID+" group by BillID order by CDate desc").c_str(),sells_select);
        if(cbShowAbBathClosed->Checked)
            SQL_exe(DBName,("select RowID,Closed,Name,BegDate,EndDate,Price,AbonementsCount,FixSum from Abonements_Bath where ClientID="+(AnsiString)ID+" order by BegDate desc,CDate desc").c_str(),abonements_bath_select);
        else
        {
            if(LastAbID)
                SQL_exe(DBName,("select RowID,Closed,Name,BegDate,EndDate,Price,AbonementsCount,FixSum from Abonements_Bath where (Closed=0 or RowID="+AnsiString(LastAbID)+") and ClientID="+(AnsiString)ID+" order by BegDate desc,CDate desc").c_str(),abonements_bath_select);
            else
                SQL_exe(DBName,("select RowID,Closed,Name,BegDate,EndDate,Price,AbonementsCount,FixSum from Abonements_Bath where Closed=0 and ClientID="+(AnsiString)ID+" order by BegDate desc,CDate desc").c_str(),abonements_bath_select);
        }
        if(SelFlag)
        {
            unsigned i;
...
void __fastcall TClientsForm::FormShow(TObject *Sender)
{
    IDs = NULL;
    Counter = 0;
    // Скопировать клубные карты если есть
    /*
    char *sql =

    "insert into ABONEMENTS_BATH(ABONEMENTID,CLIENTID,ABTYPEID,PERSONID,SERVICEID,NAME,PRICE,ABONEMENTSCOUNT,DISCOUNT,AUTOFLOW,BEGDATE,ENDDATE,CLOSED,CDATE,CUID,EDATE,EUID,FIXSUM,SYNC) "
    "select a.ROWID,a.CLIENTID,a.ABTYPEID,a.PERSONID,a.SERVICEID,a.NAME,a.PRICE,a.ABONEMENTSCOUNT,a.DISCOUNT,a.AUTOFLOW,a.BEGDATE,a.ENDDATE,a.CLOSED,a.CDATE,a.CUID,a.EDATE,a.EUID,a.FIXSUM,a.SYNC "
    "from ABONEMENTS a "
    "inner join ABTYPES b on a.ABTYPEID=b.ROWID "
    "left join ABONEMENTS_BATH c on a.ROWID=c.ABONEMENTID "

    "where c.ROWID is null and b.BATH=1";
...
//---------------------------------------------------------------------------
void __fastcall TClientsForm::MirrorClubCard(__int64 CID)
{
...
void __fastcall TClientsForm::sgClientsRowInfo(__int64 *pID, unsigned cnt)
{
    ...
    if(pID)
    {
        ...
        "where c.ROWID is null and b.BATH=1 and a.CLIENTID=";

        SQL_exe(NULL, (AnsiString(sql) + ClientID).c_str());
    }
    else
        ClientID = 0;
...
void __fastcall TClientsForm::SpeedButton4Click(TObject *Sender)
{
    ...
    SQL_exe(NULL,("insert into Reserve_Del select Reserve.*,null,null from Reserve where RowID="+retString+" or ParentID="+retString).c_str());
...
void __fastcall TClientsForm::sbResClick(TObject *Sender)
{
    ...
    */
    ...
    else if(Tag.Pos("~Баня~") > 0)
    {
        // Есть доступное посещение?

        AnsiString sFixSum, sCounter;

        SQL_exefun(DBName,("select count(*) from Abonements_Bath where Closed=0 and FixSum>=0 and ClientID="+AnsiString(ClientID)).c_str(),&sCounter);
        unsigned abs_cnt = atoi(sCounter.c_str());
        ...
        SQL_exefun(DBName,("select first 1 FixSum from Abonements_Bath where Closed=0 and FixSum>=0 and ClientID="+AnsiString(ClientID)+" order by BegDate,CDate").c_str(),&sFixSum);

        if(atof(sFixSum.c_str()) == 0.0)
        {
            // Требуется проверка на любой действующий абонемент
            SQL_exefun(DBName,("select count(*) from Abonements_Bath where Closed=0 and FixSum>=0 and EndDate>="+AnsiString((int)Now())+" and ClientID="+AnsiString(ClientID)).c_str(),&sCounter);
            abs_cnt = atoi(sCounter.c_str());
        ...
        AnsiString pRow[4];
        SQL_exefunrow(DBName,("select first 1 RowID,Price,AbonementsCount,FixSum from Abonements_Bath where Closed=0 and FixSum>=0 and ClientID="+(AnsiString)ClientID+" order by BegDate,CDate").c_str(),4,pRow);
        ...
        else
        {
            ...
            {
                SQL_CancelTransaction();
                Application->MessageBox("Внимание!\nКол-во посещений равно 0.","Ошибка БД",MB_OK);
        ...
        {
            PDEL(IDsAbonementsBath);
            sgAbonementsBathFullUpdate(ClientID,AbID);
            sgReserveFullUpdate(RID);
            sgFitSheduleFullUpdate();
        }
    }
...
void __fastcall TClientsForm::sgFitSheduleFullUpdate(bool fTop)
{
    ...
    "select a.ROWID,b.NAME,c.NAME,a.BEGDATE,a.ENDDATE,a.FITGROUPCOUNT,count(d.ROWID) "
    "from FITGROUP a "
    "left join SERVICES b on a.SERVICEID=b.ROWID "
    "left join PERSONAL c on a.PERSONID=c.ROWID "
    "left join RESERVE d on a.ROWID=d.FITGROUPID and d.CLIENTID=";

    sql += ClientID;

    sql += " where trunc(a.BEGDATE)=";
    sql += (int)FitCurrentDay + FitCurButton;
    sql += " and bin_and(b.WEEKDAYS,2)!=0 "

    "group by 1,2,3,4,5,6 "

    "order by a.BEGDATE,b.NAME";
...
//---------------------------------------------------------------------------
void __fastcall TClientsForm::sgAbonementsBathDblClick(TObject *Sender)
{
    if(sgClients->Row > 0 && Counter > 0 && sgAbonementsBath->Row > 0 && CounterAbonementsBath > 0)
    {
        if(!IDs || !IDsAbonementsBath) return;

        ClientID = IDs[sgClients->Row-1];
        //AbonementSPAID = 0;
        AbonementBathID = IDsAbonementsBath[sgAbonementsBath->Row-1];

        SQL_exefun(DBName,("select CDate from Abonements_Bath where RowID="+AnsiString(AbonementBathID)).c_str(),&retString);
        TDateTime dt = atof(retString.c_str());
        retString = dt;
...
//---------------------------------------------------------------------------
void __fastcall TClientsForm::sbBathVisitClick(TObject *Sender)
{
    // Есть доступное посещение?
    if(!ClientID) return;
    ...
    AnsiString pRow[6];
    SQL_exefunrow(DBName,("select first 1 a.RowID,a.Price,a.AbonementsCount,a.FixSum,a.Name,b.AbType from Abonements_Bath a inner join AbTypes b on a.AbTypeID=b.RowID where a.Closed=0 and a.FixSum>=0 and a.ClientID="+(AnsiString)ClientID+" order by a.BegDate,a.CDate").c_str(),6,pRow);
    ...
    */
    ...
    SQL_iniInsert(DBName,"Visits_Bath",&VisitID);
    SQL_addInsert("AbonementID",AbID);

    SQL_addInsert("VisitsDate",(double)dt);
    SQL_addInsert("BegTime",(double)(dt - (int)dt));
    SQL_addInsert("EndTime",(double)(dt - (int)dt));
...
//---------------------------------------------------------------------------
void __fastcall TClientsForm::SpeedButton5Click(TObject *Sender)
{
    ResBathForm = new TResBathForm(this);

    ResBathForm->ShowModal();
